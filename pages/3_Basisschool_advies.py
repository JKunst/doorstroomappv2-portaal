...
from components.doorstroom_functions import (
    ...
    analyze_flexible_transitions_small,
    counts_with_percentages, analyze_flexible_without_basisschooladvies,
)
from components.popups import filter_helper, filter_helper_2
...
st.markdown(
    ...
    .card {
        ...
        box-shadow: 0 4px 14px rgba(0,0,0,0.08); transition: all 0.2s ease;
        border: 1px solid rgba(0,0,0,0.05);
    }
    .card:hover { transform: translateY(-3px); box-shadow: 0 8px 22px rgba(0,0,0,0.12); }
...
all_leerfases = ['VWO', 'HAVO / VWO', 'HAVO', 'VMBO (g)t / HAVO',
        'VMBO (g)t', 'VMBO k / (g)t', 'VMBO k', 'VMBO b / k', 'VMBO b',
        'te breed', 'Onbekend']
...
st.text(
    "Gebruik de filters om een groep leerlingen te analyseren. Selecteer een basisschool advies om het verloop van de schoolcarière te zien."
    ...
    with st.spinner("Berekenen en plaatje maken..."):
        ...
        # with col1:
            ...
            leerfase_start = st.selectbox(
                "Selecteer de een basisschool advies:",
            ...
            flex_df = analyze_flexible_transitions_small(
                updated_df,
                schooljaar_start=schooljaar_start,
                schooljaar_eind=schooljaar_eind,
                leerfase_start=leerfase_start,
